  ...
  const handleScan = async (value: string) => {
    ...
    try {
      ...
      const transactionResponse = await fetch(
        `https://robotmanagerv1test.qikpod.com/nanostore/transaction?order_id=${orderRecord.id}&item_id=${value}&transaction_item_quantity=-1&transaction_type=outbound&transaction_date=${new Date().toISOString().split('T')[0]}`,
        {
          method: 'POST',
          headers: {
            'accept': 'application/json',
